...
class ADMIN
{
    ...
    public funtion adminSignin($email, $password, $csrf_token){
        try{
            if (!isset($csrf_token) || !hash_equals($_SESSION['csrf_token'], $csrf_token)){
                echo "<script>alert('Invalid CSRF token.'); window.location.href = '../../../';</script>";
                exit;
            ...
            if($stmt->rowCount() == 1 && $userRow['password'] == md5($password)){
                ...
                echo "<script>alert('INVALID'); window.location.href = '../../../index.php';</script>";
                exit;
            }
    ...
    public function adminSignout ($email, $password, $csrf_token){
